...
class Player {
    ...
    draw(ctx, time) {
        ...
        ctx.beginPath();
        ctx.ellipse(px + 4, py + 22, 20, 10, 0, 0, Math.PI * 2);
        ctx.fill();
...
class Enemy {
    ...
    draw(ctx, time) {
        ...
        ctx.globalAlpha = 1.0;
        if (isBoss) {
            const glowSize = 40 + Math.sin(time * 8) * 10;
            ctx.fillStyle = `rgba(255, 71, 87, ${0.3 + Math.sin(time * 6) * 0.2})`;
            ctx.beginPath(); ctx.arc(px, py + bob, glowSize, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = `rgba(255, 165, 2, ${0.2 + Math.sin(time * 10) * 0.15})`;
            ctx.beginPath(); ctx.arc(px, py + bob, glowSize * 0.7, 0, Math.PI * 2); ctx.fill();
            if (Math.floor(time * 20) % 3 === 0) {
                ctx.fillStyle = `rgba(255, 0, 0, 0.15)`;
                ctx.fillRect(px - 35, py - 35 + bob, 70, 70);
            }
        }
        ctx.fillStyle = 'rgba(0,0,0,0.3)';
        ctx.beginPath();
        ctx.ellipse(px + 2, py + 22, isBoss ? 32 : 20, isBoss ? 16 : 9, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.font = `${Math.floor(TILE_SIZE * (isBoss ? 1.4 : 0.7))}px serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ...
        if (isBoss) {
            ...
            ctx.fillText('BOSS', px, py - 48 + bob);
        }
        if (this.hp < this.maxHp) {
            const barW = isBoss ? 80 : 44, barH = isBoss ? 12 : 5;
            const barX = px - barW/2, barY = py - (isBoss ? 50 : 30) + bob;
            ctx.fillStyle = '#000';
        ...
        if (trophy) {
            trophy.addEventListener('click', () => {
                ...
                if (data.bossDefeated) {
                    this.audio.playCoin();
                    this.showPopup('🏆 Piala: BOSS SLAYER', 'Kamu telah mengalahkan bos dan mendapatkan piala!', 'win', false);
                } else {
                    this.audio.playError();
                    this.showPopup('🔒 Piala Terkunci', 'Kamu harus mengalahkan bos terlebih dahulu!', 'lose', true);
                }
            });
