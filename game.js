...
class GameManager {
    ...
    _setupUI() {
        ...
        bindBtn('btn-history-delete', () => {
            if (confirm('Apakah kamu yakin ingin menghapus semua riwayat permainan?')) {
                ...
                // Check if any boss win exists before clearing
                const hasBossWin = data.history && data.history.some(h => (h.level === 5 || h.isBoss) && h.won);
                data.history = [];
                // If no boss win remains in history, reset trophy to locked
                if (!hasBossWin) {
    ...
    updateTrophy() {
        ...
        if (!trophy) return;
        const data = SaveManager.load();
        // Derive unlock state from history as source of truth
        const hasBossWin = data.history && data.history.some(h => (h.level === 5 || h.isBoss) && h.won);
        const isUnlocked = hasBossWin;
        if (isUnlocked) {
    ...
    showHistory() {
        ...
        if (!data.history || data.history.length === 0) {
            ...
            // Always update trophy when history is empty (will lock if no boss win)
            this.updateTrophy();
        } else {
            if (deleteBtn) deleteBtn.style.display = 'block';
            let html = `<div class="history-header"><div>NAMA</div><div>SCORE</div><div>WAKTU</div><div>LEVEL</div></div>`;
            for (const entry of data.history) {
                const isBoss = entry.level === 5 || entry.isBoss;
                const bossClass = isBoss ? 'history-entry boss-entry' : 'history-entry';
                const bossBadge = isBoss ? (entry.won ? ' 👑 BOSS SLAYER' : ' 💀 BOSS DEFEATED') : '';
                const statusText = entry.won ? 'MENANG' : 'KALAH';
                html += `<div class="${bossClass}">
                    <div class="h-name">${entry.name}${bossBadge}</div>
                    <div class="h-score">${entry.score}</div>
                    <div class="h-time">${entry.time}</div>
                    <div class="h-level">${entry.level} - ${statusText}</div>
                </div>`;
            }
